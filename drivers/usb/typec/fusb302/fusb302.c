...
extern int dwc3_pd_vbus_ctrl(int on, bool isPRSwap);
					      ...
					      enum typec_data_role value)
{
	...
	//	return 0;
	//}
	extcon_set_cable_state_(chip->extcon, EXTCON_USB_HOST, value);
	return 0;
...
static int init_regulators(struct fusb302_chip *chip)
{
	...
	ret = regulator_set_load(chip->vdd, VDD_OPTIMUM_MODE);
	if (ret < 0) {
	...
	ret = regulator_set_load(chip->switch_vdd,
	...
	if (ret < 0) {
		...
		regulator_set_load(chip->switch_vdd, 0);
		goto disable_vdd;
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	chip->extcon = devm_extcon_dev_allocate(chip->dev, fusb302_extcon_cable);
